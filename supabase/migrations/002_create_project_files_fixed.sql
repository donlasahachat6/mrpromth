-- Migration: Create project_files table and update schema (FIXED)
-- Date: 2024-01-08
-- Purpose: Enable code editor and file management features
-- Note: workflows.id is TEXT, not UUID
...
ADD COLUMN IF NOT EXISTS active_project_id TEXT REFERENCES workflows(id) ON DELETE SET NULL;
...
BEGIN
  ...
  IF EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'workflows' AND column_name = 'project_package_url'
  ) THEN
    RAISE NOTICE 'SUCCESS: workflows columns added';
  ELSE
    RAISE EXCEPTION 'FAILED: workflows columns not added';
  END IF;
END $$;
